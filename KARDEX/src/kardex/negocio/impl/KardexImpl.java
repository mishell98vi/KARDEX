...
public class KardexImpl implements KardexI{
    ...
    public int insertar(Kardex kardex) throws Exception {
        int numFilas = 0;
        String sqlC = "INSERT INTO Kardex (codigoKardex, codigoProducto, fechaEmision, tipoTrasaccion, existecias, valortotal, cantidadEdicion) VALUES (?,?,?,?,?,?,?)";
    ...
    public int modificar(Kardex kardex) throws Exception {
        int numFilas = 0;
        String sqlC = "UPDATE kardex SET codigoKardex=?, codigoProducto=?, fechaEmision=?, tipoTrasaccion=?, existecias=?, valortotal=?, cantidadEdicion=? WHERE codigoKardex=? and codigoProducto=?";
        ArrayList<Parametro> lisParametros = new ArrayList<>();
        lisParametros.add(new Parametro(1, kardex.getCodKardex()));
        lisParametros.add(new Parametro(2, kardex.getProducto().getCodigoProducto()));
        if (kardex.getFechaEmision() instanceof java.util.Date) {
    ...
    public Kardex obtener(int codKardex, int codProducto) throws Exception {
        Kardex nKardex = null;
        String sqlC = "SELECT codigoKardex, codigoProducto, fechaEmision, tipoTrasaccion, existecias, valortotal, cantidadEdicion FROM Kardex WHERE codigoKardex=? and codigoProducto=?";
        ArrayList<Parametro> lisParametros = new ArrayList<>();
    ...
    public ArrayList<Kardex> obtener() throws Exception {
        ...
        String sqlC = "SELECT codigoKardex, codigoProducto, fechaEmision, tipoTrasaccion, existecias, valortotal, cantidadEdicion FROM Kardex";
        Conexion con = null;
        ProductoI prodDao = new ProductoImp();
        Producto nProd = null;
        Kardex nKardex=null;
    ...
    public ArrayList<Kardex> obtenerkardexProducto(int producto) throws Exception {
        ArrayList<Kardex> listKardex = new ArrayList<>();
        String sqlC = "SELECT codKardex, codProducto, fechaEmision, tipoTrasaccion, existencia, valortotal, cantEditable FROM Kardex Where codProducto=?";
